...
class distLinear(nn.Module):
    ...
    def forward(self, x,y):
        ...
        for i in range(x_normalized.size()[0]):
            ...
            weight_n = self.alpha*weight_c.repeat(self.L.weight.data.size()[0]).view(self.L.weight.data.size())
            weight_n = weight_n + self.L.weight.data.clone() 
            weight_norm = torch.norm(weight_n,p=2,dim=1).unsqueeze(-1).repeat(1,self.indim)
            weight_n = weight_n / weight_norm
            weight_n[c] = self.L.weight.data[c]
